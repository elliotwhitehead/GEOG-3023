...
This occurred because there are some missing data points in the data. We can remove all the rows with missing 
data first, then do the correlation. We will do this a couple steps, first creating our own data.frame with 
only the numbers we want to analyse. Similar with the previous case, we can select the columns we want to 
keep using `select()`. Then we use `filter()` to remove the rows with NAs (! means NOT, so !is.na() means NOT NA)
...
Although the scatter plot shows the dots are everywhere, it generally shows that as Freedom to make life choices
increases in a country, that country's life satisfaction (Life.Ladder) also increase. This is a positive correlation. 
Let’s do this again and add the best fit line, so the trend is more clear. The function to fit the line or simple 
regression is `lm()`, and the way to use it is `lm(y~x)` where `y~x` is the `formula` to be fitted, `y` is the 
response or dependent variable, `x` is the explanatory/independent variable, and `~` sign is a separator. 
Type `?lm` to see more about how to use this function.

```{r}

# first make the scatter plot
ggplot(smaller_df) + aes(x=Freedom.to.make.life.choices, y=Life.Ladder) + geom_point() +
...
In the last code block, we had ggplot fit a linear model and add that line to our plot (with some error margins)
...
Let's fit a linear model predicting Life.Ladder (target variable) using Freedom.to.make.life.choices (predictor variable)
```{r}
...
What `lm(y~x)` does is to fit the simple linear regression model `y = a + bx`. In other words, `lm()` 
can estimate the coefficients `a` (intercept) and `b` (slope). We can assess the intercept and slope like the following:
...
**Q2: (20 pts)** One of the other columns in the dataset `whr_data`, is `Social.support`. Please calculate the correlation 
coefficient between Social.support and Life.Ladder. Be careful of the `NA` values in the columns as shown earlier. 
