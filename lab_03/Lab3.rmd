...
The data frame is a commonly used data structure in statistics and RStudio, and is a key component of this class. The basic structure of a data is in the form of rows and columns. Each row is an observation, or data point. Each column is a variable, which is a measure, feature, or characteristic of that observation. For example, the mtcars dataframe we have already looked at:
...
The `nycflights` data frame is a massive trove of information. It has data from 32730 flights!! Let's think about 
...
Today, we're going to focus on analyze flight departure and arrival time delays. 
...
sd(nycflights$dep_delay)
IQR(nycflights$dep_delay)
min(nycflights$dep_delay)
max(nycflights$dep_delay)
skewness(nycflights$dep_delay)
kurtosis(nycflights$dep_delay)
```
...
## Now you're turn: Comparing departure delays between NYC airports
